...
const AqiPage = ({ data, location }) => {
    ...
    const handleSearch = (e) => {
        ...
        setIsSearching(true);
        setSearchResults([]);
        fetch(`http://localhost:5000/api/search-aqi?keyword=${searchQuery}`)
            .then(res => res.json())
...
const TripPlannerPage = () => {
    ...
    const handleFindRoute = (e) => {
        ...
        setRoutes(null);
        setRouteInfo('');
        fetch(`http://localhost:5000/api/clean-route?start=${start}&end=${destination}`)
            .then(res => res.json())
            .then(data => {
                if (data.error) throw new Error(data.error);
                setRoutes(data);
...
const AqiScreen = () => {
  ...
  useEffect(() => {
    ...
    const fetchDataForPosition = (position) => {
      ...
      fetch(`http://localhost:5000/api/live-data?lat=${latitude}&lon=${longitude}`)
        .then(res => res.ok ? res.json() : Promise.reject(`API Error: ${res.status}`))
        .then(mainData => {
